...
class GrabItApp:
    def __init__(self, page: ft.Page):
        self.page = page
        self.page.title = "Grabit"
        self.page.theme_mode = ft.ThemeMode.DARK
        self.page.bgcolor = DARK_BG
        self.page.scroll = ft.ScrollMode.ADAPTIVE
        # Mobile specific setup
        if self.page.platform not in [ft.PagePlatform.ANDROID, ft.PagePlatform.IOS]:
    ...
    def setup_ui(self):
        ...
        self.btn_cancel = ft.ElevatedButton("Cancel", on_click=self.cancel_download, bgcolor=SURFACE2, color="#ef4444", disabled=True)
        ...
        self.view_download = ft.Container(
            padding=20,
            content=ft.Column([
                ft.Text("Grabit", size=24, weight="bold", color=ACCENT),
                self.url_input,
        ...
        self.page.navigation_bar = ft.NavigationBar(
            ...
            destinations=[
                ft.NavigationBarDestination(icon="download", label="Download"),
                ft.NavigationBarDestination(icon="history", label="History"),
            ]
        )

        self.page.add(ft.SafeArea(self.content_area, expand=True))
    ...
    def start_download(self, e):
        ...
        def run_dl():
            if mode == "audio":
                # Audio: Explicitly request m4a since Android doesn't have FFmpeg to convert to mp3
                fmt = "bestaudio[ext=m4a]/bestaudio/best"
                postproc = []
            else:
                # Video: only request pre-merged formats because FFmpeg is not installed on mobile
                v_qual = "" if vid_qual == "best" else f"[height<={vid_qual}]"
                fmt = f"best[ext=mp4]{v_qual}/best{v_qual}/b"
                postproc = []
    ...
    def update_history_ui(self):
        ...
        if not self.history:
            ...
            for item in reversed(self.history):
                color = "#4ade80" if item["status"] == "Done" else "#ef4444"
...
def main(page: ft.Page):
    try:
        ...
        page.add(
            ft.Text("CRASH REPORT:", size=20, weight="bold", color="red"),
